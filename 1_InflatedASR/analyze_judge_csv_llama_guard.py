...
Inputs:
  1) A CSV produced by either:
       - `judge-style-behavior` (HF judge), with:
           - example_id
       ...
       - OR `judge_behavior_llamaguard.py` (Llama Guard 2), with:
           - example_id
           - style
           - prompt
           - response
           - lg2_label (SAFE / UNSAFE / UNKNOWN)
           - lg2_categories, lg2_raw, is_jailbreak (optional)
...
Note:
  When using Llama Guard 2, we map:
    SAFE   → REFUSE  (模型安全地拒绝有害指令)
    UNSAFE → COMPLY  (模型给出了有害帮助，相当于越狱成功)
  然后复用原来的 refusal / comply 统计逻辑。
"""
...
def load_judge_csv(judge_csv: str) -> pd.DataFrame:
    """
    加载行为评估结果 CSV，并统一出一列 'judge_label' ∈ {REFUSE, COMPLY, UNKNOWN}。

    支持两种来源：
      1) 旧版 judge-style-behavior: 直接有 judge_label
      2) Llama Guard 2 版: 使用 lg2_label (SAFE / UNSAFE / UNKNOWN)，映射为:
           SAFE   → REFUSE
           UNSAFE → COMPLY
    """
    df = pd.read_csv(judge_csv)

    has_judge_label = "judge_label" in df.columns
    has_lg2_label = "lg2_label" in df.columns

    if has_lg2_label:
        # 使用 Llama Guard 2 的结果
        print("[Load] Detected Llama Guard 2 columns (lg2_label); mapping SAFE/UNSAFE → REFUSE/COMPLY.")
        mapping = {
            "SAFE": "REFUSE",     # 安全 → 视作“拒绝有害请求”
            "UNSAFE": "COMPLY",   # 不安全 → 视作“顺从有害请求”（越狱成功）
        }
        # 统一成大写再映射
        lg2_upper = df["lg2_label"].astype(str).str.upper()
        df["judge_label"] = lg2_upper.map(mapping).fillna("UNKNOWN")

    elif has_judge_label:
        # 旧版 HF judge，直接用
        print("[Load] Detected HF judge_label; using REFUSE/COMPLY directly.")
        df["judge_label"] = df["judge_label"].astype(str).str.upper()

    else:
        raise ValueError(
            "Input CSV must contain either 'judge_label' (HF judge) "
            "or 'lg2_label' (Llama Guard 2)."
        )
    ...
    print("[Load] judge_label (standardized) value_counts:")
...
def compute_style_refusal_stats(df_clean: pd.DataFrame,
    ...
    Here, judge_label=REFUSE 意味着：
      - 原 HF judge: 模型拒绝有害指令
      - Llama Guard 2: 模型被判 SAFE（我们映射为 REFUSE）
    """
...
def compute_per_example_effect(df_clean: pd.DataFrame,
    ...
    在 Llama Guard 2 语义下，这等价于：
      - improved: base=UNSAFE → style=SAFE
      - degraded: base=SAFE → style=UNSAFE
...
def main():
    ...
    parser.add_argument(
        ...
        help="CSV produced by judge-style-behavior or judge_behavior_llamaguard "
             "(must contain example_id, style, prompt, response, and judge_label or lg2_label)."
    )
    parser.add_argument(
    ...
    # 1) Load & clean (自动适配 HF judge / Llama Guard 2)
