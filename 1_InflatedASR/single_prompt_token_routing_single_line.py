...
def plot_prompt_routing_two_lines(
    ...
    x_sample = np.array(display_layers, dtype=float)                # x 上的真实层号
    ...
    # 3) 插值，画顺滑曲线
    y_orig_dense = np.interp(x_dense, x_sample, y_orig_sample)
    y_style_dense = np.interp(x_dense, x_sample, y_style_sample)
